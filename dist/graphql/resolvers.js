...
export const resolvers = {
    Query: {
        ...
        searchMovies: async (_, arg, context) => {
            authCheck(context);
    ...
    Mutation: {
        ...
        addFavorite: async (_, { imdbID }, context) => {
            authCheck(context);
            const movie = await upsertMovie(imdbID);
            //upsert the Favorite
            const favorite = await Favorite.findOneAndUpdate({ user: context.user?.id, movie: movie.id }, { $setOnInsert: { user: context.user?.id, movie: movie.id } }, { upsert: true, new: true });
            return favorite;
        },
        removeFavorite: async (_, { imdbID }, context) => {
            authCheck(context);
            if (!imdbID)
                throw new GraphQLError('imdbID is required', {
